...
class OutputAnalyzer {
    private final HeartRate activity;
    ...
    OutputAnalyzer(HeartRate activity, TextureView graphTextureView, Handler mainHandler) {
...
//                                1f * (measurementLength - millisUntilFinished - clipLength) / 1000f);
                        float b = 60f * (detectedValleys - 1) / (Math.max(1, (valleys.get(valleys.size() - 1) - valleys.get(0)) / 1000f));
                        String currentValue = String.valueOf((double) Math.round(b * 10) / 10);
                        sendMessage(HeartRate.MESSAGE_UPDATE_REALTIME, currentValue);
                    }
            ...
            public void onFinish() {
                ...
                if (valleys.size() == 0) {
                    mainHandler.sendMessage(Message.obtain(
                            mainHandler,
                            HeartRate.MESSAGE_CAMERA_NOT_AVAILABLE,
...
//                        1f * (valleys.get(valleys.size() - 1) - valleys.get(0)) / 1000f);
                ...
                int a = detectedValleys - 1;

                float c = 1f * (valleys.get(valleys.size() - 1) - valleys.get(0)) / 1000f;
                sendMessage(HeartRate.MESSAGE_UPDATE_REALTIME, currentValue);
                ...
                sendMessage(HeartRate.MESSAGE_UPDATE_FINAL, returnValueSb.toString());

                cameraService.stop();
            }
        };

        //     activity.setViewState(HeartRate.VIEW_STATE.MEASUREMENT);
        timer.start();
    }
