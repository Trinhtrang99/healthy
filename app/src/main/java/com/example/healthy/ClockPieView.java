...
public class ClockPieView extends View {
    ...
    private final int GRAY_COLOR = Color.parseColor("#D7D7DB");
    ...
    public ClockPieView(Context context, AttributeSet attrs) {
        super(context, attrs);
          ...
          mRectF = new RectF(mViewWidth / 2 , mViewHeight / 2 - 360,
                mViewWidth / 2 + 360, mViewHeight / 2 + 360);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(cirRect, 180f, -90f, false, redPaint);
    }

    private void drawBackground(Canvas canvas) {
        for (int i = 0; i < 12; i++) {
            if (i % 2 != 0) {
                tempPoint.set(pieCenterPoint.x - (int) (Math.sin(Math.PI / 12 * i) * (pieRadius + 20
                        + lineLength)), pieCenterPoint.y - (int) (Math.cos(Math.PI / 12 * i) * (
                        pieRadius + 20
                                + lineLength)));
                tempPointRight.set(pieCenterPoint.x + (int) (Math.sin(Math.PI / 12 * i) * (pieRadius + 20
                        + lineLength)), pieCenterPoint.y + (int) (Math.cos(Math.PI / 12 * i) * (
                        pieRadius + 20
                                + lineLength)));

            } else {
                tempPoint.set(pieCenterPoint.x - (int) (Math.sin(Math.PI / 12 * i) * (pieRadius + 50
                        + lineLength)), pieCenterPoint.y - (int) (Math.cos(Math.PI / 12 * i) * (
                        pieRadius + 50
                                + lineLength)));
                tempPointRight.set(pieCenterPoint.x + (int) (Math.sin(Math.PI / 12 * i) * (pieRadius + 50
                        + lineLength)), pieCenterPoint.y + (int) (Math.cos(Math.PI / 12 * i) * (
                        pieRadius + 50
                                + lineLength)));
        ...
        canvas.drawCircle(pieCenterPoint.x, pieCenterPoint.y, pieRadius - 70, whitePaint);
        canvas.drawText("6.5", pieCenterPoint.x, mViewHeight, textPaint);
    }
